...
#' @param rate the risk-free rate for each date.
#' @param divRate the dividend rate for (the underlying of) each options.
#' 
...
addOptions <- function(stocksData, optionNames=NULL, underlyingNames,
                    Date=NULL, impVol, strike, moneyness, callput, maturity, rate, divRate=0)
{
  n <- stocksData$N
  ...
  for(i in seq_along(optionNames)) {
    premium[,optionNames[i]] <- BSPrice(stocksData$price[,underlyingNames[i]], 
                                        ...
                                        rate[,2], impVol[,optionNames[i]], 
                                        maturity[i], callput[i], 
                                        divRate[,optionNames[i]])
